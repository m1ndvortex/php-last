...
class OfflineService {
    ...
    this.db = await openDB(this.DB_NAME, this.DB_VERSION, {
      upgrade(db) {
        ...
        if (!db.objectStoreNames.contains('offlineForms')) {
          const formsStore = db.createObjectStore('offlineForms', { keyPath: 'id' });
  ...
  async cacheData(type: OfflineData['type'], id: string, data: any): Promise<void> {
  ...
  async addPendingSync(method: PendingSync['method'], url: string, data: any): Promise<string> {
    if (!this.db) await this.init();
