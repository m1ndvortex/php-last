...
export function usePerformanceMonitoring(componentName: string) {
  ...
  const logMetrics = () => {
    ...
    console.log(
      `👆 Interaction Time: ${metrics.value.interactionTime.toFixed(2)}ms`,
    );

    if (memoryInfo) {
      console.log(
        `💾 Memory Usage: ${(memoryInfo.used / 1024 / 1024).toFixed(2)}MB`,
      );
      console.log(
        `💾 Memory Total: ${(memoryInfo.total / 1024 / 1024).toFixed(2)}MB`,
      );
    }
...
export function useApiPerformanceMonitoring() {
  ...
  const measureApiCall = async <T>(
    ...
    try {
      ...
      if (duration > 1000) {
        console.warn(
          `🐌 Slow API call detected: ${apiName} took ${duration.toFixed(2)}ms`,
        );
      }
      ...
      console.error(
        `❌ API call failed: ${apiName} (${duration.toFixed(2)}ms)`,
        error,
      );
      throw error;
    }
