...
export function useGoldPricing() {
  ...
  const calculateItemPrice = (params: GoldPricingParams): PriceBreakdown => {
    ...
    if (weight <= 0 || goldPricePerGram <= 0 || quantity <= 0) {
      throw new Error('Weight, gold price per gram, and quantity must be greater than zero')
    }
  ...
  const validatePricingParams = (params: Partial<GoldPricingParams>): Record<string, string> => {
  ...
  const formatCurrency = (amount: number): string => {
    const locale = settingsStore.businessConfig?.default_language === 'fa' ? 'fa-IR' : 'en-US'
