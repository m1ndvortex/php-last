...
export const useAccountingStore = defineStore("accounting", () => {
  ...
  const filteredAccounts = computed(() => {
    return accounts.value.filter((account) => {
      const matchesType =
        !accountFilter.value.type || account.type === accountFilter.value.type;
      const matchesActive = account.is_active === accountFilter.value.is_active;
      const matchesSearch =
        !accountFilter.value.search ||
        account.name
          .toLowerCase()
          .includes(accountFilter.value.search.toLowerCase()) ||
        account.code
          .toLowerCase()
          .includes(accountFilter.value.search.toLowerCase());
  ...
  const filteredTransactions = computed(() => {
    return transactions.value.filter((transaction) => {
      const matchesDateFrom =
        !transactionFilter.value.date_from ||
        transaction.transaction_date >= transactionFilter.value.date_from;
      const matchesDateTo =
        !transactionFilter.value.date_to ||
        transaction.transaction_date <= transactionFilter.value.date_to;
      const matchesType =
        !transactionFilter.value.type ||
        transaction.type === transactionFilter.value.type;
      const matchesLocked =
        transactionFilter.value.is_locked === null ||
        transaction.is_locked === transactionFilter.value.is_locked;
      const matchesSearch =
        !transactionFilter.value.search ||
        transaction.description
          .toLowerCase()
          .includes(transactionFilter.value.search.toLowerCase()) ||
  ...
  const fetchTransactions = async () => {
    try {
      ...
      if (transactionFilter.value.date_from)
        params.append("date_from", transactionFilter.value.date_from);
      if (transactionFilter.value.date_to)
        params.append("date_to", transactionFilter.value.date_to);
      if (transactionFilter.value.account_id)
        params.append(
          "account_id",
          transactionFilter.value.account_id.toString(),
        );
      if (transactionFilter.value.type)
        params.append("type", transactionFilter.value.type);
      if (transactionFilter.value.is_locked !== null)
        params.append(
          "is_locked",
          transactionFilter.value.is_locked.toString(),
        );
      if (transactionFilter.value.search)
        params.append("search", transactionFilter.value.search);
  ...
  const unlockTransaction = async (id: number) => {
    try {
      ...
      error.value =
        err.response?.data?.message || "Failed to unlock transaction";
      throw err;
    } finally {
      loading.value = false;
    }
  ...
  const fetchCostCenters = async () => {
    try {
      ...
      error.value =
        err.response?.data?.message || "Failed to fetch cost centers";
      throw err;
    } finally {
      loading.value = false;
    }
  ...
  const fetchAuditLogs = async () => {
    try {
      loading.value = true;
      const params = new URLSearchParams();
      if (auditLogFilter.value.date_from)
        params.append("date_from", auditLogFilter.value.date_from);
      if (auditLogFilter.value.date_to)
        params.append("date_to", auditLogFilter.value.date_to);
      if (auditLogFilter.value.action)
        params.append("action", auditLogFilter.value.action);
      if (auditLogFilter.value.auditable_type)
        params.append("auditable_type", auditLogFilter.value.auditable_type);
      if (auditLogFilter.value.user_id)
        params.append("user_id", auditLogFilter.value.user_id.toString());
      if (auditLogFilter.value.search)
        params.append("search", auditLogFilter.value.search);

      const response = await api.get(
        `/accounting/audit-logs?${params.toString()}`,
      );
  ...
  const getGeneralLedger = async (
    ...
    try {
      ...
      const response = await api.get(
        `/accounting/accounts/${accountId}/ledger?${params.toString()}`,
      );
      return response.data.data;
    } catch (err: any) {
      error.value =
