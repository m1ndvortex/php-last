...
export const useDashboardStore = defineStore('dashboard', () => {
  ...
  const getKPIByKey = computed(() => (key: string) => 
    kpis.value.find(kpi => kpi.key === key)
  );

  const getWidgetById = computed(() => (id: string) => 
  ...
  const fetchAlerts = async () => {
    try {
      ...
      console.warn('Failed to fetch alerts from API, using default data:', error);
