      ...
      <div v-else-if="data && data.length > 0" class="space-y-4">
        ...
        <div class="grid grid-cols-2 gap-4">
          <div class="bg-yellow-50 dark:bg-yellow-900/20 p-3 rounded-lg">
            <div class="text-sm font-medium text-yellow-800 dark:text-yellow-400 mb-1">
          ...
          >
            <div class="flex items-center space-x-3">
              <div class="flex-shrink-0">
                <div class="w-3 h-3 rounded-full" :style="{ backgroundColor: getPurityColor(purity.gold_purity) }"></div>
              </div>
              <div>
            ...
            <div class="text-right">
              ...
              <div class="text-xs" :class="[
                purity.margin_percentage >= 20 ? 'text-green-600 dark:text-green-400' :
                purity.margin_percentage >= 10 ? 'text-yellow-600 dark:text-yellow-400' :
...
const totalWeightSold = computed(() => {
  ...
  return data.value.reduce((total, purity) => total + (purity.total_weight_sold || 0), 0)
})

const averageMargin = computed(() => {
  if (!data.value?.length) return 0
  const totalMargin = data.value.reduce((total, purity) => total + purity.margin_percentage, 0)
  ...
  chart.value = new Chart(ctx, {
    ...
    data: {
      ...
      datasets: [{
        ...
        backgroundColor: chartData.map(purity => getPurityColor(purity.gold_purity)),
        borderWidth: 2,
        borderColor: '#ffffff'
      }]
    },
