      ...
      <div v-else-if="displayData.length > 0" class="space-y-4">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div class="bg-blue-50 dark:bg-blue-900/20 p-3 rounded-lg">
            <div
              class="text-sm font-medium text-blue-800 dark:text-blue-400 mb-1"
            >
          ...
          <div class="bg-purple-50 dark:bg-purple-900/20 p-3 rounded-lg">
            <div
              class="text-sm font-medium text-purple-800 dark:text-purple-400 mb-1"
            >
            ...
            >
              ...
              <div class="text-right">
                ...
                <div
                  class="text-xs"
                  :class="[
                    category.margin_percentage >= 20
                      ? 'text-green-600 dark:text-green-400'
                      : category.margin_percentage >= 10
                        ? 'text-yellow-600 dark:text-yellow-400'
                        : 'text-red-600 dark:text-red-400',
                  ]"
                >
                  {{ formatPercentage(category.margin_percentage) }}
                  {{ $t("dashboard.margin") }}
                </div>
              </div>
            </div>
...
const topRevenue = computed(() => {
  ...
  return displayData.value.reduce(
    (max: CategoryData, category: CategoryData) =>
...
const bestMargin = computed(() => {
  ...
  return displayData.value.reduce(
    (max: CategoryData, category: CategoryData) =>
      category.margin_percentage > (max?.margin_percentage || 0)
        ? category
        : max,
  );
});
...
const createChart = () => {
  ...
  chart.value = new Chart(ctx, {
    type: "bar",
    data: {
      labels: chartData.map((category: CategoryData) =>
        getCategoryName(category),
      ),
      datasets: [
        {
          ...
          data: chartData.map(
            (category: CategoryData) => category.total_revenue,
          ),
