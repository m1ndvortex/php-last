      ...
      <div class="mt-3">
        ...
        <form @submit.prevent="scheduleReport" class="space-y-4">
          ...
          <div>
            ...
            <select v-model="form.frequency" required class="form-select w-full">
          ...
          <div>
            ...
            <select v-model="form.deliveryMethod" required class="form-select w-full">
              <option value="email">{{ $t('reports.email') }}</option>
              <option value="download">{{ $t('reports.download_link') }}</option>
            </select>
          </div>
  ...
  try {
    await api.post('/reports/schedule', {
      ...
      delivery: {
        ...
        recipients: form.value.deliveryMethod === 'email' 
