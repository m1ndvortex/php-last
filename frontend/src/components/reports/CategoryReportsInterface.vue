<template>
  <div class="space-y-6">
    ...
    <div class="bg-white dark:bg-gray-800 shadow rounded-lg">
      <div class="px-4 py-5 sm:p-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <div
            v-if="selectedReportType === 'gold_purity_analysis'"
            class="md:col-span-2"
          >
            <label class="form-label">{{
              $t("reports.gold_purity_range")
            }}</label>
            <div class="flex space-x-2">
              <input
          ...
          <div v-if="selectedReportType === 'category_sales_performance'">
            ...
            <select v-model="filters.group_by" class="form-input">
              ...
              <option value="main_category">
                {{ $t("reports.main_categories_only") }}
              </option>
              <option value="subcategory">
                {{ $t("reports.subcategories_only") }}
              </option>
            </select>
          </div>
        </div>
    ...
    <div v-if="reportData" class="space-y-6">
      ...
      <div
        v-if="selectedReportType === 'category_hierarchy'"
      ...
      <div
        v-if="selectedReportType === 'category_sales_performance'"
        class="bg-white dark:bg-gray-800 shadow rounded-lg"
      >
        <div class="px-4 py-5 sm:p-6">
          <h3 class="text-lg font-medium text-gray-900 dark:text-white mb-4">
          ...
          <div class="overflow-x-auto">
            ...
            >
              ...
              >
                <tr
                  v-for="item in reportData.performance_data"
                  :key="
                    item.category?.id ||
                    `${item.main_category?.id}-${item.subcategory?.id}`
                  "
                >
                  <td class="table-cell">
                    <div v-if="filters.group_by === 'both'">
                      <div class="font-medium">
                        {{ item.main_category?.name || "N/A" }}
                      </div>
      ...
      <!-- Stock Levels Report -->
      <div
        v-if="selectedReportType === 'category_stock_levels'"
        class="bg-white dark:bg-gray-800 shadow rounded-lg"
      >
        <div class="px-4 py-5 sm:p-6">
          <h3 class="text-lg font-medium text-gray-900 dark:text-white mb-4">
          ...
          <div class="border-b border-gray-200 dark:border-gray-700 mb-4">
            <nav class="-mb-px flex space-x-8">
              ...
              >
                {{ $t(level.label) }} ({{
                  reportData.stock_levels[level.key]?.length || 0
                }})
              </button>
            </nav>
          </div>
          ...
          <div class="overflow-x-auto">
            ...
            >
              ...
              >
                <tr
                  v-for="item in reportData.stock_levels[activeStockLevel]"
                  :key="item.id"
                >
                  <td class="table-cell">
                  ...
                  <td class="table-cell">
                    <span
                      :class="[
                        ...
                        item.quantity === 0
                          ? 'bg-red-100 text-red-800 dark:bg-red-900/20 dark:text-red-400'
                          : item.quantity <= (filters.low_stock_threshold || 10)
                            ? 'bg-yellow-100 text-yellow-800 dark:bg-yellow-900/20 dark:text-yellow-400'
                            : 'bg-green-100 text-green-800 dark:bg-green-900/20 dark:text-green-400',
                      ]"
                    >
        ...
        <div class="px-4 py-5 sm:p-6">
          ...
          <div class="overflow-x-auto">
            ...
            >
              ...
              >
                <tr
                  v-for="group in reportData.purity_groups"
                  :key="group.purity"
                >
                  <td class="table-cell">
                  ...
                  <td class="table-cell">
                    {{ formatWeight(group.total_weight) }}
                  </td>
                  <td class="table-cell">
                    {{ formatCurrency(group.total_inventory_value) }}
                  </td>
                  <td class="table-cell">
                    {{ formatCurrency(group.sales_revenue) }}
                  </td>
                  <td class="table-cell">
                    <span
                      :class="[
                        'px-2 py-1 text-xs rounded-full',
                        group.margin_percentage >= 20
                          ? 'bg-green-100 text-green-800 dark:bg-green-900/20 dark:text-green-400'
                          : group.margin_percentage >= 10
                            ? 'bg-yellow-100 text-yellow-800 dark:bg-yellow-900/20 dark:text-yellow-400'
                            : 'bg-red-100 text-red-800 dark:bg-red-900/20 dark:text-red-400',
                      ]"
                    >
                  ...
                  <td class="table-cell">
                    <span
                      :class="[
                        'px-2 py-1 text-xs rounded-full',
                        group.turnover_rate >= 0.8
                          ? 'bg-green-100 text-green-800 dark:bg-green-900/20 dark:text-green-400'
                          : group.turnover_rate >= 0.5
                            ? 'bg-yellow-100 text-yellow-800 dark:bg-yellow-900/20 dark:text-yellow-400'
                            : 'bg-red-100 text-red-800 dark:bg-red-900/20 dark:text-red-400',
                      ]"
                    >
...
const availableSubcategories = computed(() => {
  ...
  return subcategories.value.filter(
    (cat: Category) => cat.parent_id == filters.value.main_category_id,
  );
});
...
const loadCategories = async () => {
  try {
    ...
    mainCategories.value = allCategories.filter(
      (cat: Category) => !cat.parent_id,
    );
    subcategories.value = allCategories.filter(
      (cat: Category) => cat.parent_id,
    );
  } catch (error) {
