<template>
  <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6">
    ...
    <div class="space-y-4">
      <!-- Main content area -->
      <div v-if="variant === 'chart'" class="h-64 flex items-center justify-center">
        <SkeletonLoader width="100%" height="100%" variant="card" />
      </div>
      
      <div v-else-if="variant === 'stats'" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
      ...
      <div v-else-if="variant === 'list'" class="space-y-3">
        <div v-for="i in listItems" :key="`item-${i}`" class="flex items-center space-x-3">
      ...
      <div v-else class="space-y-3">
        <SkeletonLoader v-for="i in lines" :key="`line-${i}`" :width="getLineWidth(i)" height="16px" />
      </div>
    </div>

    <!-- Footer -->
    <div v-if="showFooter" class="mt-6 pt-4 border-t border-gray-200 dark:border-gray-700">
