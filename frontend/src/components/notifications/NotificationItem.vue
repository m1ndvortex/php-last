  ...
  >
    ...
    >
      ...
      <div class="flex-1 min-w-0">
        ...
        <div
          v-if="shouldShowAdditionalData(notification)"
        ...
        >
          ...
          <div v-else-if="notification.type === 'reminder'" class="space-y-1">
            <div v-if="notification.data?.age">
              <span class="font-medium"
                >{{ $t("notifications.data.age") }}:</span
              >
              {{ notification.data.age }}
            </div>
            <div v-if="notification.data?.years_married">
              <span class="font-medium"
                >{{ $t("notifications.data.yearsMarried") }}:</span
              >
          ...
          <div
            v-else-if="notification.type === 'communication'"
            class="space-y-1"
          >
            <div v-if="notification.data?.communication_type">
              <span class="font-medium"
                >{{ $t("notifications.data.type") }}:</span
              >
              {{
                $t(
                  `communications.types.${notification.data.communication_type}`,
                )
              }}
            </div>
            <div v-if="notification.data?.customer_name">
              <span class="font-medium"
                >{{ $t("notifications.data.customer") }}:</span
              >
...
const getNotificationIcon = (type: string, subtype: string) => {
  switch (type) {
    ...
    case "reminder":
      if (subtype.includes("birthday") || subtype.includes("anniversary"))
        return HeartIcon;
      return BellIcon;
    case "system":
      return CogIcon;
