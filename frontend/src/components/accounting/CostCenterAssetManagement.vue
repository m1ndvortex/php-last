      ...
      <div class="bg-white dark:bg-gray-800 shadow rounded-lg overflow-hidden">
        <div class="overflow-x-auto">
          ...
          >
            ...
            >
              <tr
                v-for="costCenter in accountingStore.costCenters"
                :key="costCenter.id"
              >
                <td
          ...
          <div>
            ...
            >
              ...
              <option value="under_maintenance">
                {{ $t("accounting.under_maintenance") }}
              </option>
            </select>
          </div>
          <div>
...
const filteredAssets = computed(() => {
  return accountingStore.assets.filter((asset) => {
    const matchesCategory =
      !assetFilter.value.category ||
      asset.category === assetFilter.value.category;
    const matchesStatus =
      !assetFilter.value.status || asset.status === assetFilter.value.status;
    const matchesSearch =
      !assetFilter.value.search ||
      asset.name
        .toLowerCase()
        .includes(assetFilter.value.search.toLowerCase()) ||
      asset.asset_number
        .toLowerCase()
        .includes(assetFilter.value.search.toLowerCase());
...
const getLocalizedName = (costCenter: CostCenter) => {
  ...
  return locale === "fa" && costCenter.name_persian
    ? costCenter.name_persian
    : costCenter.name;
};
