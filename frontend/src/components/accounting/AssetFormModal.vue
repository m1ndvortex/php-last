<template>
  <div class="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50">
    <div class="relative top-10 mx-auto p-5 border w-11/12 md:w-3/4 lg:w-1/2 shadow-lg rounded-md bg-white dark:bg-gray-800 max-h-screen overflow-y-auto">
      ...
      <form @submit.prevent="handleSubmit" class="space-y-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            ...
            <select
              ...
              class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:text-white sm:text-sm"
            >
              <option value="straight_line">{{ $t('accounting.straight_line') }}</option>
              <option value="declining_balance">{{ $t('accounting.declining_balance') }}</option>
              <option value="units_of_production">{{ $t('accounting.units_of_production') }}</option>
            </select>
          </div>
        </div>
        ...
        <div class="flex justify-end space-x-3 pt-6 border-t border-gray-200 dark:border-gray-700">
          ...
          >
            {{ loading ? $t('common.saving') : (asset ? $t('common.update') : $t('common.create')) }}
          </button>
        </div>
      </form>
    </div>
