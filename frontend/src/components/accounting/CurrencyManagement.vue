            ...
            <tr v-for="currency in currencies" :key="currency.code">
              <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900 dark:text-white">
                <div class="flex items-center">
                  ...
                  <span v-if="currency.code === baseCurrency" class="ml-2 inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-blue-100 text-blue-800 dark:bg-blue-900 dark:text-blue-200">
                    {{ $t('accounting.base') }}
                  </span>
                </div>
              </td>
              ...
              <td class="px-6 py-4 whitespace-nowrap text-sm text-right text-gray-900 dark:text-white font-mono">
                {{ currency.code === baseCurrency ? '1.000000' : currency.rate.toFixed(6) }}
              </td>
    ...
    <div class="bg-white dark:bg-gray-800 shadow rounded-lg p-6">
      ...
      <div class="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
        ...
        <div>
          ...
          <div class="mt-1 block w-full px-3 py-2 bg-gray-50 border border-gray-300 rounded-md text-sm text-gray-900 dark:bg-gray-600 dark:border-gray-500 dark:text-white font-mono">
    ...
    <div class="bg-white dark:bg-gray-800 shadow rounded-lg overflow-hidden">
      ...
      <div class="overflow-x-auto">
        <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
          ...
          <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
            <tr v-for="balance in multiCurrencyBalances" :key="`${balance.account_code}-${balance.currency}`">
              <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white">
                <div>
                  <p class="font-medium">{{ balance.account_code }}</p>
                  <p class="text-gray-500 dark:text-gray-400">{{ balance.account_name }}</p>
                </div>
              </td>
          ...
          <tfoot class="bg-gray-50 dark:bg-gray-700">
            <tr>
              ...
              <td class="px-6 py-3 text-sm font-bold text-right text-gray-900 dark:text-white font-mono">
...
const currencies = ref<Currency[]>([
  { code: 'USD', name: 'US Dollar', rate: 1.000000, change: 0.0000, is_active: true },
  { code: 'EUR', name: 'Euro', rate: 0.850000, change: -0.0125, is_active: true },
  { code: 'IRR', name: 'Iranian Rial', rate: 42000.000000, change: 0.0050, is_active: true },
  { code: 'AED', name: 'UAE Dirham', rate: 3.673000, change: 0.0000, is_active: true }
])

const multiCurrencyBalances = ref<MultiCurrencyBalance[]>([
  { account_code: '1001', account_name: 'Cash - USD', currency: 'USD', balance: 10000, base_equivalent: 10000 },
  { account_code: '1002', account_name: 'Cash - EUR', currency: 'EUR', balance: 5000, base_equivalent: 5882.35 },
  { account_code: '1003', account_name: 'Cash - IRR', currency: 'IRR', balance: 50000000, base_equivalent: 1190.48 },
  { account_code: '1004', account_name: 'Cash - AED', currency: 'AED', balance: 15000, base_equivalent: 4084.97 }
])
...
const totalBaseEquivalent = computed(() => {
  return multiCurrencyBalances.value.reduce((sum, balance) => sum + balance.base_equivalent, 0)
})
...
const convertCurrency = () => {
  const fromRate = currencies.value.find(c => c.code === converter.value.fromCurrency)?.rate || 1
  const toRate = currencies.value.find(c => c.code === converter.value.toCurrency)?.rate || 1
...
const updateMultiCurrencyBalances = () => {
  multiCurrencyBalances.value.forEach(balance => {
    const rate = currencies.value.find(c => c.code === balance.currency)?.rate || 1
