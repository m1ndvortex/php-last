        ...
        <div class="bg-white dark:bg-gray-800 shadow rounded-lg">
          <div class="px-6 py-4 border-b border-gray-200 dark:border-gray-700">
            <div class="flex items-center justify-between">
              <div>
                <h4 class="text-lg font-medium text-gray-900 dark:text-white">
                  {{ selectedAccount ? `${selectedAccount.code} - ${getLocalizedName(selectedAccount)}` : $t('accounting.select_account') }}
                </h4>
                <p v-if="selectedAccount" class="text-sm text-gray-500 dark:text-gray-400">
                  {{ $t('accounting.current_balance') }}: {{ formatCurrency(selectedAccount.current_balance || 0) }}
                </p>
              </div>
            ...
            <div v-if="ledgerEntries.length > 0" class="overflow-x-auto">
              <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                ...
                <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                  <tr v-for="entry in ledgerEntries" :key="`${entry.date}-${entry.reference}`">
