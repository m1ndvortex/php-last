  ...
  <div class="fixed inset-0 z-50 overflow-y-auto">
    <div class="flex items-center justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
      ...
      <div class="inline-block align-bottom bg-white dark:bg-gray-800 rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-5xl sm:w-full">
        ...
        <!-- PDF Preview Content -->
        <div class="bg-gray-100 dark:bg-gray-900 p-6 max-h-[80vh] overflow-y-auto">
            ...
            <div class="bg-white shadow-lg rounded-lg overflow-hidden">
              ...
              <div v-else class="p-8 space-y-6">
                ...
                <div class="flex justify-between items-start">
                  <div>
                    <div class="w-32 h-16 bg-gray-200 border-2 border-dashed border-gray-300 flex items-center justify-center mb-4">
                  ...
                  <div class="text-right">
                    <div class="text-sm text-gray-600 space-y-1">
                      <p><strong>{{ invoice?.language === 'fa' ? 'شماره فاکتور:' : 'Invoice #:' }}</strong> {{ invoice?.invoice_number }}</p>
                      <p><strong>{{ invoice?.language === 'fa' ? 'تاریخ صدور:' : 'Issue Date:' }}</strong> {{ formatDate(invoice?.issue_date) }}</p>
                      <p><strong>{{ invoice?.language === 'fa' ? 'تاریخ سررسید:' : 'Due Date:' }}</strong> {{ formatDate(invoice?.due_date) }}</p>
                    </div>
                  </div>
                </div>
                ...
                <div class="grid grid-cols-2 gap-8">
                  <div>
                    <h3 class="text-sm font-semibold text-gray-900 mb-2">
                      {{ invoice?.language === 'fa' ? 'صادر شده برای:' : 'Bill To:' }}
                    </h3>
                  ...
                  <div>
                    ...
                    <div class="text-sm text-gray-700">
                      <p class="font-medium">{{ $t("business.company_name") }}</p>
                  ...
                  <table class="w-full border-collapse border border-gray-300">
                    <thead>
                      <tr class="bg-gray-50">
                        ...
                        <th class="border border-gray-300 px-4 py-2 text-right text-sm font-semibold text-gray-900">
                          {{ invoice?.language === 'fa' ? 'تعداد' : 'Qty' }}
                        </th>
                        <th class="border border-gray-300 px-4 py-2 text-right text-sm font-semibold text-gray-900">
                          {{ invoice?.language === 'fa' ? 'قیمت واحد' : 'Unit Price' }}
                        </th>
                        <th class="border border-gray-300 px-4 py-2 text-right text-sm font-semibold text-gray-900">
                    ...
                    <tbody>
                      <tr v-for="item in invoice?.items" :key="item.id">
                        ...
                        <td class="border border-gray-300 px-4 py-2 text-sm text-gray-900 text-right">
                          {{ formatNumber(item.quantity) }}
                        </td>
                        <td class="border border-gray-300 px-4 py-2 text-sm text-gray-900 text-right">
                          {{ formatCurrency(item.unit_price) }}
                        </td>
                        <td class="border border-gray-300 px-4 py-2 text-sm font-medium text-gray-900 text-right">
                      ...
                      <div class="border-t border-gray-300 pt-2">
                        <div class="flex justify-between text-base font-bold">
                          <span class="text-gray-900">
                            {{ invoice?.language === 'fa' ? 'مجموع نهایی:' : 'Total:' }}
                          </span>
                          <span class="text-gray-900">
                ...
                <!-- Notes -->
                <div v-if="invoice?.notes" class="border-t border-gray-300 pt-4">
                  <h3 class="text-sm font-semibold text-gray-900 mb-2">
