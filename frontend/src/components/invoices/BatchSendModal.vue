      ...
      >
        ...
        >
          <div class="flex items-center justify-between">
            ...
            >
              {{ $t("invoices.batch_send_invoices") }}
            </h3>
            <button
              @click="$emit('close')"
        ...
        <div class="px-6 py-4">
          <p class="text-sm text-gray-600 dark:text-gray-400 mb-4">
            {{ $t("invoices.batch_send_description") }}
          </p>

          <!-- Not implemented notice -->
          <div class="text-center py-8">
            <PaperAirplaneIcon class="mx-auto h-12 w-12 text-yellow-400" />
        ...
        >
          ...
          >
            {{ $t("invoices.send_invoices") }} ({{ $t("common.coming_soon") }})
          </button>
          <button
            @click="$emit('close')"
...
const handleSend = () => {
  ...
  const result = {
    success: true,
    message: "Batch invoice sending started",
    job_id: "batch-send-123",
  };
